...
//        User user = getUser(); // 当前登录的用户信息
    ...
    public void export(HttpServletResponse response) throws IOException {
        ...
        for (User user : all) {
            ...
            row1.put("真实姓名", user.getRealName());
            row1.put("年龄", user.getAge());
            row1.put("性别", user.getSex());
            row1.put("地址", user.getAddress());
            row1.put("电话号码", user.getPhone());
            row1.put("票类型", user.getType());
            row1.put("身份证号码", user.getIdentify());
            list.add(row1);
        }
    ...
    public Result<?> upload(MultipartFile file) throws IOException {
        ...
        for (List<Object> row : lists) {
            ...
            user.setRealName(row.get(1).toString());
            ...
            user.setType(row.get(5).toString());
            user.setIdentify(row.get(5).toString());
            saveList.add(user);
        }
